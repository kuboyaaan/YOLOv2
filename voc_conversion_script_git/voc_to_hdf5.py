...
parser.add_argument(
    ...
    default='~/documents/yolo/yad2k/yad2k/voc_conversion_scripts/data')
...
def get_boxes_for_id(voc_path, image_id):
    ...
    boxes : array of int
        ...
        5xN array.
    """
    fname = os.path.join(voc_path, 'VOC/Annotations/{}.xml'.format(image_id))
...
def get_ids(voc_path, datasets):
    ...
    for image_set in datasets:
        id_file = os.path.join(voc_path, 'VOC/ImageSets/Main/{}.txt'.format(image_set))
...
def _main(args):
    ...
    add_to_dataset(voc_path, val_ids, val_images, val_boxes)
    print('Processing Pascal VOC 2007 test set.')
    add_to_dataset(voc_path,test_ids, test_images, test_boxes)
